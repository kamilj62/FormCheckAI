...
def get_locks(state: RouterState) -> list[dict]:
    ...
    # V54: strong squat evidence outranks false pull-up or muscle-up
    # protections in this specific squat/bodyweight conflict pattern.
    squat_back_over_false_bodyweight = (
        state.raw_label == "squat"
        and float(state.raw_conf or 0.0) >= 0.95
        and state.bio_label == "push_press"
        and state.squat_label == "overhead_squat"
        and 0.75 <= float(state.squat_conf or 0.0) < 0.80
        and state.protected_label in {"pull_up", "muscle_up"}
        and state.protected_reason in {
    ...
    if squat_back_over_false_bodyweight:
        _add_lock(
            locks,
            label="squat_back",
            confidence=max(
            ...
            reason="squat_back_over_false_bodyweight_lock_v54",
            priority=149,
        )

    # V55: an explosive overhead split pattern can indicate snatch even
    # when the Olympic router confuses it with clean-and-jerk.
    snatch_over_false_cj_overhead_pattern = (
        state.olympic_label == "clean_and_jerk"
        and 0.60 <= float(state.olympic_conf or 0.0) <= 0.70
        and float(state.explosive_score or 0.0) >= 75.0
        and float(state.wrist_overhead or 0.0) >= 0.50
        and bool(state.looks_split)
        and bool(state.looks_thruster)
        and not bool(state.looks_clean)
    ...
    if snatch_over_false_cj_overhead_pattern:
        _add_lock(
            locks,
            label="snatch",
            confidence=max(
            ...
            reason="snatch_over_false_cj_overhead_pattern_v55",
            priority=150,
        )

    # V56: squat biomechanics with low overhead exposure outrank a false
    # thruster protection when press and Olympic evidence are both weak.
    squat_back_over_false_thruster = (
        state.raw_label == "push_press"
        and float(state.raw_conf or 0.0) < 0.60
        and state.bio_label == "squat"
        and float(state.bio_conf or 0.0) >= 0.70
        and state.squat_label in {"squat_back", "squat_front"}
        and float(state.squat_conf or 0.0) >= 0.95
        and state.protected_label == "thruster"
        and state.protected_reason == "thruster_pattern_detected"
        and float(state.olympic_conf or 0.0) < 0.60
        and float(state.wrist_overhead or 0.0) < 0.20
        and not bool(state.looks_clean)
    ...
    if squat_back_over_false_thruster:
        _add_lock(
            locks,
            label="squat_back",
            confidence=max(
            ...
            reason="squat_back_over_false_thruster_lock_v56",
            priority=151,
        )
