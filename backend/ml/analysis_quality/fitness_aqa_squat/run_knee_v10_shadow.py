...
from ml.analysis_quality.fitness_aqa_squat.build_knee_v10_resampled_features import (
    build_feature_names,
    build_vector,
    geometry,
)
...
DEFAULT_MODEL = (
    ...
    / "knee_complete_rep_v10.joblib"
)

CONSERVATIVE_FORWARD_THRESHOLD = 0.525


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run V10 knee-fault models in shadow mode "
...
def main():
    ...
    if not model_path.exists():
        raise SystemExit(
            f"V10 model not found: {model_path}"
        )
    ...
    if generated_feature_names != model_feature_names:
        raise RuntimeError(
            "V10 feature ordering does not match "
    ...
    for chunk_index, chunk in enumerate(
        ...
        for detected in detected_reps:
            ...
            if vector.shape != (
                ...
                raise RuntimeError(
                    "Unexpected V10 feature-vector shape: "
                    f"{vector.shape}"
                )

            if not np.all(np.isfinite(vector)):
                raise RuntimeError(
                    "Non-finite V10 feature value detected"
                )
