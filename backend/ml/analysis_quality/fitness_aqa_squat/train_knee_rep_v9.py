...
def load_split(split):
    data_path = BASE / f"knee_v9_rep_{split}.jsonl"
    metadata_path = BASE / f"knee_v9_rep_{split}_metadata.json"
...
def main():
    ...
    if not (
        ...
        raise RuntimeError(
            "V9 feature ordering differs across splits"
        )
    ...
    bundle = {
        "version": "v9_complete_rep_ascent_targets",
    ...
    model_path = (
        MODEL_DIR / "knee_complete_rep_v9.joblib"
    )

    joblib.dump(bundle, model_path)

    write_predictions(
        BASE / "knee_rep_v9_validation_predictions.jsonl",
        validation["records"],
        validation["forward"],
        validation["inward"],
        validation_forward_scores,
    ...
    write_predictions(
        BASE / "knee_rep_v9_test_predictions.jsonl",
        test["records"],
        test["forward"],
        test["inward"],
        test_forward_scores,
