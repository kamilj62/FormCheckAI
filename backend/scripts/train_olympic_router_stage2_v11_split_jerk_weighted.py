...
DATA_PATH = Path(
    "ml/router_rebuild_v1/features/router_features_v6_grouped_split_jerk_augmented.csv"
)
MODEL_OUT = Path(
    "app/models/candidates/olympic_router_stage2_v11_split_jerk_weighted.joblib"
)
REPORT_OUT = Path(
    "ml/router_rebuild_v1/reports/olympic_router_stage2_v11_split_jerk_weighted_metrics.json"
)
PREDICTIONS_OUT = Path(
    "ml/router_rebuild_v1/reports/olympic_router_stage2_v11_split_jerk_weighted_predictions.csv"
)
...
print("\nTraining sample weights by class:")
for label in sorted(set(y_train)):
    mask = y_train == label
    print(
        label,
        round(float(train_sample_weights[mask][0]), 6),
        f"rows={int(mask.sum())}",
    )
...
bundle = {
    ...
    "random_state": RANDOM_STATE,
    "architecture": "stage_2_olympic_router_v11_grouped_cj",
}
