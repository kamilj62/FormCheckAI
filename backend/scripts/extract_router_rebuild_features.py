...
def extract_video(path: Path) -> tuple[np.ndarray, dict]:
    ...
    if len(features) != 80:
        raise RuntimeError(f"Expected 80 features, got {len(features)}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--features-out",
        default="ml/router_rebuild_v1/features/router_features_v1.csv",
    )
    parser.add_argument(
        "--errors-out",
        default="ml/router_rebuild_v1/features/router_feature_errors_v1.csv",
    )
